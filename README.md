...
- MCP tools для подключения аккаунта, просмотра баланса/прогноза, пополнения и отчётов
...
После любого из способов можно вызывать MCP tools без повторной передачи логина/пароля.
...
- `connect_selectel_account` — сохранить учётные данные Selectel для пользователя
- `get_balance` — получить текущие балансы аккаунта
- `get_balance_prediction` — получить прогноз достаточности баланса
- `top_up_balance_saved_card` — пополнить баланс с сохранённой карты (с подтверждением `confirm=true`)
- `top_up_balance_new_card` — создать ссылку на оплату новой картой
- `top_up_balance_sbp` — создать платёж через СБП (QR + ссылка)
- `top_up_balance_bank_transfer` — сформировать ссылку на платёжное поручение (PDF)
- `check_payment_status` — проверить статус асинхронного платежа по `transaction_id`
- `get_billing_report_by_project` — получить отчёт по услугам/расходам за период с фильтрами
...
Показывает текущее состояние баланса аккаунта Selectel: основной, бонусный и VK-баланс по каждому договору, сумму задолженности и итоговый доступный остаток. Учётные данные берутся из БД, повторно вводить пароль не нужно.

Параметры не обязательны — достаточно пустого запроса:

```json
{}
```

Если в `.env` задан `DEFAULT_USER_ID`, этого достаточно. Для нескольких пользователей можно явно указать `user_id`:
...
Оценивает, на сколько хватит текущего баланса при текущем потреблении — по категориям услуг (облачные серверы, объектное хранилище, VMware, VPC). API: `GET /v2/billing/prediction` ([документация Selectel](https://docs.selectel.ru/api/balance/)).
...
Опционально можно передать `card_id`; фактическое списание выполняется только с `confirm: true`.
...
Возвращает ссылку платёжного шлюза, где пользователь вводит реквизиты карты.
...
Возвращает `transaction_id`, ссылку и/или QR для оплаты в банковском приложении.

### `top_up_balance_bank_transfer`
...
Возвращает ссылку на PDF платёжного поручения.
...
### `get_billing_report_by_project`

Обязательные параметры: `start`, `end`, `locale`.
